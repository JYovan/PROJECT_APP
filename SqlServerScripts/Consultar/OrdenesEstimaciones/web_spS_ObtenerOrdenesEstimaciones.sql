...
BEGIN
	...
	dbo.Sucursales.DireccionZona AS Zona, 
	dbo.Sucursales.CR
	FROM            
	dbo.OrdenesEstimaciones AS OE INNER JOIN
	dbo.Clientes AS cli ON OE.Cliente = cli.ID AND 
	OE.Mov IN ('Mesa de reporte', 'Estimacion') AND 
	OE.MovEnLinea = 1 AND 
	OE.Estatus IN ('CONCLUIDO', 'PENDIENTE', 'BORRADOR') AND 
	OE.Facturado <> 1 OR
	OE.Mov IN ('Mesa de reporte', 'Estimacion') AND 
	OE.MovEnLinea = 0 AND 
	OE.Estatus IN ('BORRADOR') AND 
	OE.Facturado <> 1 
	LEFT JOIN
